...
struct trait {
    ...
    virtual void collect_viewport_shapes(
        struct scene_object*,
        frame_state*,
        const mat4&                  T,
        bool                         selected,
    ...
    // only called when a trait is removed from an object while running. Only the destructor will be
    // called at exit
...
class scene {
  ...
  public:
    ...
    scene(
        ...
        : trait_factories(trait_factories), root(root), selected_object(root),
...
struct transform_trait : public trait {
    ...
    transform_trait(trait_factory* p, vec3 t, quat r, vec3 s)
        : translation(t), rotation(r), scale(s), trait(p) {}
...
struct transform_trait_factory : public trait_factory {
    struct create_info {
        ...
        create_info(vec3 t = vec3(0.f), quat r = quat(0.f, 0.f, 0.f, 1.f), vec3 s = vec3(1.f))
    ...
    void add_to(scene_object* obj, void* ci) override {
        ...
        obj->traits[id()] = std::make_unique<transform_trait>(
            this,
            c == nullptr ? vec3(0.f) : c->translation,
            c == nullptr ? quat(0.f, 0.f, 0.f, 1.f) : c->rotation,
            c == nullptr ? vec3(1.f) : c->scale
        );
    }
};
...
struct light_trait : public trait {
    ...
    light_trait(trait_factory* f, light_type t, vec3 p, vec3 c)
        : trait(f), type(t), param(p), color(c) {}
...
struct light_trait_factory : public trait_factory {
    ...
    void add_to(scene_object* obj, void* ci) override {
        auto c            = ((create_info*)ci);
        obj->traits[id()] = std::make_unique<light_trait>(
            this,
            c == nullptr ? light_type::directional : c->type,
            c == nullptr ? vec3(1.f, 0.f, 0.f) : c->param,
            c == nullptr ? vec3(1.f) : c->color
        );
    }
};
...
struct camera_trait_factory : public trait_factory {
    ...
    void add_to(scene_object* obj, void* ci) override {
        auto c = ((create_info*)ci);
        obj->traits[id()]
            = std::make_unique<camera_trait>(this, c != nullptr ? c->fov : pi<float>() / 4.0f);
    }
};
