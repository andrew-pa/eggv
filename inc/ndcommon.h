...
struct mesh_header {
    ...
    mesh_header(
        ...
        : vertex_ptr(vp), index_ptr(ip), name_ptr(np), hull_ptr(hp), num_vertices(nv),
          num_indices(ni), name_length(nl), material_index(mi), aabb_min(m), aabb_max(x) {}
};
...
const trait_id TRAIT_ID_USER       = 0x8000'0000;
