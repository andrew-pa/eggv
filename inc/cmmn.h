...
struct ray {
  public:
    ...
    ray(vec3 _e = vec3(0.f), vec3 _d = vec3(0.f)) : e(_e), d(_d) {}

    // calculate the position along the ray at t
    inline vec3 operator()(float t) const { return e + d * t; }
...
struct aabb {
  public:
    ...
    aabb(vec3 m, vec3 x) : _min(m), _max(x) {}

    // create an AABB that is the union of AABBs a and b
    aabb(const aabb& a, const aabb& b) : _min(), _max() {
        add_point(a._min);
        add_point(a._max);
        add_point(b._min);
        add_point(b._max);
    ...
    inline bool contains(vec3 p) const {
        if(p.x >= _min.x && p.x <= _max.x && p.y >= _min.y && p.y <= _max.y && p.z >= _min.z
           && p.z <= _max.z)
            return true;
        return false;
    }
    ...
    inline bool inside_of(const aabb& b) const {
        return b.contains(vec3(_min.x, _min.y, _min.z)) ||
               ...
               b.contains(vec3(_max.x, _min.y, _min.z)) || b.contains(vec3(_min.x, _max.y, _min.z))
               || b.contains(vec3(_min.x, _min.y, _max.z)) ||

               b.contains(vec3(_min.x, _max.y, _max.z)) || b.contains(vec3(_max.x, _min.y, _max.z))
    ...
    inline vec3 center() const { return (_min + _max) * 1.f / 2.f; }

    inline vec3 extents() const { return _max - _min; }
...
inline json serialize(vec3 v) { return {v.x, v.y, v.z}; }

inline json serialize(vec4 v) { return {v.x, v.y, v.z, v.w}; }
...
inline vec3 deserialize_v3(const json& j) { return vec3(j.at(0), j.at(1), j.at(2)); }

inline vec4 deserialize_v4(const json& j) { return vec4(j.at(0), j.at(1), j.at(2), j.at(4)); }

enum class viewport_shape_type { box, axis };
...
struct viewport_shape {
    ...
    viewport_shape(
        viewport_shape_type ty = viewport_shape_type::axis, vec3 col = vec3(1.), mat4 T = mat4(1)
    )
