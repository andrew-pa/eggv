...
struct user_trait : public trait {
    value_handle state;
    user_trait(trait_factory* p, value_handle state)
        : trait(p), state(std::move(state)) {}
    ...
    void collect_viewport_shapes(scene_object* ob, frame_state* fs, const mat4& T, bool selected, std::vector<viewport_shape>& shapes) override;
};
...
struct user_trait_factory : public trait_factory {
    ...
    user_trait_factory(const std::shared_ptr<runtime>& rt, value args)
        : rt(rt),
            trait_subid(to_int(assoc(args, rt->symbol("id")))),
            ...
            collect_viewport_shapes_fn(rt->handle_for(assoc(args, rt->symbol("viewport-shapes"))))
    {
        ...
        while(deps != NIL) {
            ...
            if(type_of(dep) == value_type::sym) {
                trait_id res;
                if(dep == rt->symbol("transform")) res = TRAIT_ID_TRANSFORM;
                else if(dep == rt->symbol("light")) res = TRAIT_ID_LIGHT;
                else if(dep == rt->symbol("camera")) res = TRAIT_ID_CAMERA;
                else if(dep == rt->symbol("mesh")) res = TRAIT_ID_MESH;
                else if(dep == rt->symbol("rigid-body")) res = TRAIT_ID_RIGID_BODY;
                else {/* TODO: error */}
            } else {
    ...
    trait_id id() const override {
        return TRAIT_ID_USER & trait_subid;
    }
    ...
    bool dependencies_loaded(scene_object* obj, const json& unloaded_trait) override {
        return std::all_of(std::begin(this->dependencies), std::end(this->dependencies),
            [&](auto t) {
                return obj->traits[t] != nullptr;
            }
        );
    }
};
...
void user_trait::update(scene_object* obj, frame_state* fs) {
    ...
    *this->state = rt->apply(*p->update_fn,
            rt->cons(rt->make_extern_reference(obj), rt->cons(*this->state)));
}
...
void user_trait::append_transform(scene_object* obj, mat4& T, frame_state* fs) {
    ...
    value new_T = rt->apply(*p->append_transform_fn,
            rt->cons(rt->make_extern_reference(obj),
                rt->cons(fs->t,
                    rt->cons(fs->dt,
                        rt->cons(rt->from_fvec(16, &T[0][0]), rt->cons(*this->state))))));
...
void user_trait::build_gui(scene_object* obj, frame_state* fs) {
    ...
    *this->state = rt->apply(*p->build_gui_fn,
            rt->cons(rt->make_extern_reference(obj), rt->cons(*this->state)));
}

void user_trait::collect_viewport_shapes(scene_object* ob, frame_state* fs, const mat4& T, bool selected, std::vector<viewport_shape>& shapes) {
    auto* p = (user_trait_factory*)this->parent;
    auto rt = p->rt;
    if(*p->collect_viewport_shapes_fn == NIL) return;
    value lshapes = rt->apply(*p->collect_viewport_shapes_fn,
            rt->cons(rt->make_extern_reference(ob),
                rt->cons(*this->state,
                        rt->cons(rt->from_bool(selected)))));

    while(lshapes != NIL) {
        value shape = first(lshapes);
        viewport_shape_type ty;
        if(first(shape) == rt->symbol("axis")) ty = viewport_shape_type::axis;
        else if(first(shape) == rt->symbol("box")) ty = viewport_shape_type::box;
        else {/*error!*/}
...
void eggv_app::init_script_runtime() {
    ...
    std::cout << "initializing script runtime created " << (hfo.new_size - hfo.old_size) << "b of garbage\n";
}
