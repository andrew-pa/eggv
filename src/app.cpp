...
void DestroyDebugReportCallbackEXT(
    VkInstance instance, VkDebugReportCallbackEXT callback, VkAllocationCallbacks* pAllocator
) {
...
app::app(const std::string& title, vec2 winsize) {
    ...
    glfwSetErrorCallback([](int ec, const char* em) {
        if(ec == 865540) return;  // "invalid scancode"
    ...
    wnd = glfwCreateWindow(
        (int)floor(winsize.x), (int)floor(winsize.y), title.c_str(), nullptr, nullptr
    );
    if(!wnd) {
        glfwTerminate();
    ...
    glfwSetMouseButtonCallback(wnd, [](GLFWwindow* wnd, int button, int action, int mods) {
        auto t = (app*)glfwGetWindowUserPointer(wnd);
        for(auto& ih : t->input_handlers) {
            ih->mouse_button_handler(
                t, (mouse_button)button, (input_action)action, (input_mod)mods
            );
        }
    });
    ...
    CreateDebugReportCallbackEXT(
        (VkInstance)instance, (VkDebugReportCallbackCreateInfoEXT*)&cbco, nullptr, &report_callback
    );
#endif

    VkSurfaceKHR sf;
    auto         res = glfwCreateWindowSurface((VkInstance)instance, wnd, nullptr, &sf);
...
void app::run(bool pdfps) {
    ...
    while(glfwWindowShouldClose(wnd) == GLFW_FALSE) {
        ...
        if(!image_index.ok()
           && (image_index.err() == vk::Result::eErrorOutOfDateKHR
        ...
        vk::SubmitInfo         sfo{
            1,
            &swapchain->image_ava_sp.get(),
            wait_stages,
            1,
            &cb,
            1,
            &swapchain->render_fin_sp.get()};
