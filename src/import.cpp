...
json convert_scene_node(
    json&                           sc,
    aiNode*                         node,
    const char*                     geometry_source_path,
    const aiScene*                  in_scene,
    const std::vector<uuids::uuid>& matix_to_id
) {
    ...
    for(auto i = 0; i < node->mNumChildren; ++i) {
        obj["c"].push_back(
            convert_scene_node(sc, node->mChildren[i], geometry_source_path, in_scene, matix_to_id)
        );
    }
    if(node->mNumMeshes == 1) {
        obj["t"][std::to_string(TRAIT_ID_MESH)] = json{
            ...
            {"ix",      node->mMeshes[0]                                                       },
            {"mat",
             uuids::to_string(matix_to_id[in_scene->mMeshes[node->mMeshes[0]]->mMaterialIndex])}
        };
    } else {
        for(auto i = 0; i < node->mNumMeshes; ++i) {
            auto mesh_ch = make_scene_object(
                std::string(node->mName.C_Str()) + "/" + std::to_string(node->mMeshes[i])
            );
            mesh_ch["t"][std::to_string(TRAIT_ID_MESH)] = json{
                {"geo_src", geometry_source_path},
                {"ix",      node->mMeshes[i]    },
                {"mat",
                 uuids::to_string(matix_to_id[in_scene->mMeshes[node->mMeshes[i]]->mMaterialIndex]
                 )                              }
            };
...
int main(int argc, char* argv[]) {
    ...
    if(input_path == nullptr || output_path == nullptr) {
        std::cout << "usage: eggv_import [options] -i [input scene path] -o [output mesh data "
                     "path] (-scene [output scene json path])\n";
    ...
    imp.ReadFile(
        input_path,
        aiProcess_GenBoundingBoxes | aiProcessPreset_TargetRealtime_MaxQuality | aiProcess_FlipUVs
    );
    ...
    for(size_t mesh_ix = 0; mesh_ix < scene->mNumMeshes; ++mesh_ix) {
        ...
        for(size_t i = 0; i < mesh->mNumVertices; ++i) {
            geom_file::vertex v(
                ...
                has_tex_coords ? vec2(mesh->mTextureCoords[0][i].x, mesh->mTextureCoords[0][i].y)
                               : vec2(0.f)
            );
        ...
        if(gen_hulls) {
            ...
            auto hull
                = hullgen.getConvexHull((float*)mesh->mVertices, mesh->mNumVertices, false, 0.01f);
    ...
    if(write_scene) {
        ...
        out_scene["graph"] = convert_scene_node(
            out_scene, scene->mRootNode, output_path, scene, material_index_to_id
        );
                ...
                if(light->mType == aiLightSource_DIRECTIONAL
                   || light->mType == aiLightSource_POINT) {
                    ...
                    auto light_objp = std::find_if(
                        std::begin(out_scene["graph"]["c"]),
                        std::end(out_scene["graph"]["c"]),
                        [&](const auto& obj) { return obj["name"] == light->mName.C_Str(); }
                    );
                    ...
                    if(light->mType == aiLightSource_DIRECTIONAL) {
                        ...
                        std::cout << "\t\t" << light->mAttenuationConstant << ", "
                                  << light->mAttenuationLinear << ", "
                                  << light->mAttenuationQuadratic << " " << light->mSize.x << "@"
                                  << light->mSize.y << "\n";
                        light_obj["t"][std::to_string(TRAIT_ID_LIGHT)] = json{
                            {"t", light_type::point                                      },
        ...
        if(scene->HasCameras()) {
            for(auto i = 0; i < scene->mNumCameras; ++i) {
                ...
                auto cam_objp = std::find_if(
                    std::begin(out_scene["graph"]["c"]),
                    std::end(out_scene["graph"]["c"]),
                    [&](const auto& obj) { return obj["name"] == cam->mName.C_Str(); }
                );
